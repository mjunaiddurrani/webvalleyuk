		...
		formSubmitAjax: function( $form ) {
			...
			args.success = function( json ) {
				...
				if ( json.data.confirmation ) {
					...
					var $checkConatinerID_banner = $container.is("#wpforms-171");
					var $checkConatinerID_package = $container.is("#wpforms-353");
					var $checkConatinerID_customForm = $container.is("#wpforms-397");
					var $checkConatinerID_wpdev = $container.is("#wpforms-892");

					var $checkConatinerID_value = '';
					if($checkConatinerID_banner === true || $checkConatinerID_package === true || $checkConatinerID_customForm === true || $checkConatinerID_wpdev === true){
						$checkConatinerID_value = true;
					} else {
						$checkConatinerID_value = false;
					}

					if ($checkConatinerID_value === true) {
						$container.find('.wpforms-submit-container').after(json.data.confirmation);
						$container.find('form input').val('');
						$container.find('form input').attr('disabled', 'true');
						$container.find('form button[type="submit"]').attr('disabled', 'true');
						$container.find('form button[type="button"]').attr('disabled', 'true');
                        $container.find('form button[type="submit"]').addClass('form-btn-disabled');
						$container.find('form textarea').val('');
						$container.find('form textarea').attr('disabled', 'true');
